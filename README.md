...
Assurez-vous que le fichier `trainers.json` (placé dans le même dossier) associe bien les ID des dresseurs affrontés en jeu au nom correspondant dans la colonne A de l'onglet **Trainer Tracking** de la Sheet.
...
Ensure that the `trainers.json` file (placed in the same folder) accurately maps the IDs of the trainers fought in-game to their corresponding names in column A of the **Trainer Tracking** tab in your Sheet.
